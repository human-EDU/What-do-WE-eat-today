...
<jsp:include page="../common/category.jsp" />
		<div id="align">
			<div class="wrap">
				<input class="select" placeholder="검색어 입력">
							...
							<c:forEach items="${ad}" var="a">
								<li>
									<a href="/store/detail/${a.sno}" class="inner">
										<div class="img">
											<img onerror="this.src='/resources/assets/images/noimage.png'" src="${a.logo}">
										</div>
										<div class="con">
	...
	<script>
		$(function() {
			...
			function getStoreStr(store) {
				...
				str += '		<div class="img">';
				str += '			<img onerror="this.src=' + noimage + '" src="' + store.logo + '">';
				str += '		</div>';
				str += '		<div class="con">';
				str += '			<div class="ttl">' + store.name + '</div>';
				str += '			<div class="txt">';
				str += '				<p>★</p>';
				str += '				<em>' + store.scope + '</em><span>리뷰 ' + store.reviewNum + '</span><span>사장님 댓글 ' + store.replyNum + '</span>';
				str += '			</div>';
			...
			// 일반음식점 목록 띄우기
			function showStoreList(lastNo, amount, condition) {
				var param = {category: category, lastNo: lastNo, amount: amount, condition: condition}
			...
			showStoreList(lastNo, amount, condition);
			...
			$(document).scroll(function() {
				...
				if(dh == wh + wst) {
					...
					var param = {category:category, lastNo:lastNo};
					scrollService.getStoreList(param, function(result) {
						var str = '';
						for(var i in result) {
							str += getStoreStr(result[i]);
